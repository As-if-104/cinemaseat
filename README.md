...
1. Seat A4 held by `user-A` → `success: true`, `holdExpiresAt` ~10s out
2. Immediately after: seat map shows A4 as `HELD`
3. 15 seconds later: seat map shows **no** seats in `HELD` state — A4 auto-released by the background hold-expiry worker
4. `user-B` holds the same seat → `success: true` — proving it's genuinely bookable again, not just a status-field change
